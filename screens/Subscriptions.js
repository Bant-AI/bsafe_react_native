...
export default function Subscriptions({ navigation }) {
  ...
  subsRef.get().then((querySnapshot) => {
  ...
  //       fetch("https://firestore.googleapis.com/v1/projects/bant-ai/databases/(default)/documents/subscriptions/"+documentId+"?key=AIzaSyBL6jwaEBlafkAnQJrCXTNML1di26Dq_q4?currentDocument.exists=true&updateMask.fieldPaths=documentId", requestOptions)
  ...
  function subscribe(code, id) {
    if (code === id) {
      ...
      fetch(
        "https://firestore.googleapis.com/v1/projects/bant-ai/databases/(default)/documents/subscriptions?key=AIzaSyBL6jwaEBlafkAnQJrCXTNML1di26Dq_q4",
        requestOptions
      )
        .then((response) => response.text())
        // .then(result => console.log(result))
